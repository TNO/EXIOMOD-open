...
Positive variables
    ...
    RK(cnt,sec)         return to capital sectoral level
...
EQY(cnt)..    Y(cnt) =E=
                   ...
                   + ( sum(sec,KZ(cnt,sec)/YZ(cnt)*K(cnt,sec)*RK(cnt,sec))
...
EQPD(cnt,sec)$(XDZ(cnt,sec)).. PD(cnt,sec)*XD(cnt,sec)
                   *(1 - txd(cnt,sec))
                    =E=
                   K(cnt,sec)*KZ(cnt,sec)/XDZ(cnt,sec)*(RK(cnt,sec)+delta(cnt,sec)*PI(cnt))
...
EQK(cnt,sec)$(KZ(cnt,sec)).. K(cnt,sec)
                   ...
                   *(gammaK(cnt,sec)/(RK(cnt,sec)+delta(cnt,sec)*PI(cnt)))**sigmaKL(cnt,sec)
...
EQPKL(cnt,sec)$(KLZ(cnt,sec))..  PKL(cnt,sec) =E= 1/aKL(cnt,sec)*(
                  ...
                  + (gammaK(cnt,sec)**sigmaKL(cnt,sec)*(RK(cnt,sec)
...
EQRGD(cnt)..       RGD(cnt) =E= sum(sec,RK(cnt,sec)*K(cnt,sec)*KZ(cnt,sec))
...
* this mapping has been changed as compared to static version
    EQK.RK
    EQRGD.RGD
...
Parameters
    walras
    P_index(cnt,com,t)          domestic sales prices of commodities and price of leisure
    PD_index(cnt,sec,t)         domestic producer prices of commodities
    PDDE_index(cnt,com,t)       price of goods produced domestically
    PXDDE_index(cnt,com,t)      price of goods produced domestically and purchased domestically
    PDD_index(cnt,sec,com,t)    price of domestic variety
    ER_index                  exchange rate
    INDEX_index(cnt,t)          consumer price index
    PI_index(cnt,t)             price of investments private

    PEROW_index(cnt,com,t)      price of exports to ROW in national currency
    PMROW_index(cnt,com,t)      import price of imports from ROW in local currency

    PE_index(cnt,com,t)         price of exports
    PM_index(cnt,com,t)         import price of imports

    PMT_index(cnt,com,t)        import price of imports
    PET_index(cnt,com,t)        import price of imports

    PL_index(cnt,t)             domestic price of labor
    PKL_index(cnt,sec,t)        price of capital labour bundle
    RK_index(cnt,sec,t)         return to capital
    RGD_index(cnt,t)            nominal interest rate

    LS_index(cnt,t)             labor supply (exogenous)

    X_index(cnt,com,t)          domestic sales (domestic+foreign origin)
    XD_index(cnt,sec,t)         gross domestic output
    XDD_index(cnt,sec,com,t)    domestic output

    XDDE_index(cnt,com,t)       domestic production delivered to domestic market
    XXD_index(cnt,com,t)        total demand for commodity produced within the country

    TMX_index(cnt,com,t)        commodity consumed for prod of transp and trade marins

    EROW_index(cnt,com,t)       exports to RoW
    MROW_index(cnt,com,t)       imports from RoW

    E_index(cnt,com,t)          exports
    M_index(cnt,com,t)          imports

    TRADE_index(com,cnt,cntt,t) international trade in FOB prices

    IT_index(cnt,t)             total investments private

    K_index(cnt,sec,t)          capital input (exogenous)
    L_index(cnt,sec,t)          labor input

    C_index(cnt,com,t)          demand for consumer goods and leisure
    CBUD_index(cnt,t)           consumer expenditure commodities

    Y_index(cnt,t)              household income
    SH_index(cnt,t)             household savings
    SG_index(cnt,t)             government savings
    SROW_index(cnt,t)           savings of or from RoW (exogenous)
    ST_index(cnt,t)             national savings
    I_index(cnt,com,t)          demand for investment goods private

    CG_index(cnt,com,t)         intermediate public demand for goods
    TAXR_index(cnt,t)           tax revenues

    TRF_index(cnt,t)            total transfers of government to households (exogenous)

    TRROW_index(cnt,t)          total transfers to government from or to ROW

    HTRROW_index(cnt,t)         total transfers to households from or to ROW

    GDP_index(cnt,t)            gross domestic product (real)
    GDPC_index(cnt,t)           gross domestic product (nominal)
    GDPDEF_index                GDP deflator (exogenous-numeraire)

    PTM_index(cnt,t)            composite price of trade and transport margin

    SV_index(cnt,com,t)         changes in stocks
    IO_index(cnt,com,sec,t)     intermediate use of commodities

;

* =========== Recursive dynamic part : as in the Monash model ==================
Parameters
RORLAG(cnt,sec)
RORLAGZ(cnt,sec)
alphaROR(cnt,sec)
INV(cnt,sec)           new investments
;

Parameter check(cnt,sec) ;

*Loop(t$(ord(t) le 5),
Loop(t,

* Investment dynamic part

RORLAG(cnt,sec)$KZ(cnt,sec) = -1+(RK.L(cnt,sec)/PI.L(cnt) + 1-delta(cnt,sec))
                  /(1+RGD.L(cnt)/GDPDEF.L)   ;

RORLAGZ(cnt,sec)$KZ(cnt,sec)= -1+(RKZ(cnt,sec)/PIZ(cnt) + 1-delta(cnt,sec))
                  /(1+RGDZ(cnt)/GDPDEFZ)     ;

alphaROR(cnt,sec)$KZ(cnt,sec) = exp((RORLAG(cnt,sec)-RORLAGZ(cnt,sec))*((growthz(cnt)
                    +0.06+delta(cnt,sec))/(0.06*growthz(cnt)+0.06*delta(cnt,sec)))) ;

INV(cnt,sec)$KZ(cnt,sec) = K.L(cnt,sec)*KZ(cnt,sec)*(alphaROR(cnt,sec)*(growthz(cnt)+0.06+1)
                   *(growthz(cnt)+delta(cnt,sec))-0.06*(delta(cnt,sec)-1))
                   /(0.06+alphaROR(cnt,sec)*(growthz(cnt)+delta(cnt,sec)))
                   -(1-delta(cnt,sec))*K.L(cnt,sec)*KZ(cnt,sec) ;

INV(cnt,sec)$KZ(cnt,sec)    = INV(cnt,sec)*sum(com, I.L(cnt,com)*IZ(cnt,com))
                  /sum(secc, INV(cnt,secc))  ;

Display INV ;

* ========  Exogenously fixed: capital endowments

    K.FX(cnt,sec)$KZ(cnt,sec) =  ( (1-delta(cnt,sec))*K.L(cnt,sec)*KZ(cnt,sec)
                       + INV(cnt,sec))/KZ(cnt,sec) ;

*    K.FX(cnt,sec)$KZ(cnt,sec)   = (1+growthz(cnt))**(ord(t))  ;


*======== Include initial (equilibrium) levels for the endogenous variables====

    LSZ(cnt)             =  LSZ(cnt)*(1+growthz(cnt))    ;
    LSZZ(cnt)            =  LSZZ(cnt)*(1+growthz(cnt))   ;

    XZ(cnt,com)          = XZ(cnt,com)*(1+growthz(cnt))  ;
    XDZ(cnt,sec)         = XDZ(cnt,sec)*(1+growthz(cnt))  ;
    XDDZ(cnt,sec,com)    = XDDZ(cnt,sec,com)*(1+growthz(cnt))  ;

    XDDEZ(cnt,com)       = XDDEZ(cnt,com)*(1+growthz(cnt))  ;
    XXDZ(cnt,com)        = XXDZ(cnt,com)*(1+growthz(cnt)) ;

    TMXZ(cnt,com)        = TMXZ(cnt,com)*(1+growthz(cnt)) ;

    EROWZ(cnt,com)       = EROWZ(cnt,com)*(1+growthz(cnt)) ;
    MROWZ(cnt,com)        = MROWZ(cnt,com)*(1+growthz(cnt)) ;

    EZ(cnt,com)          = EZ(cnt,com)*(1+growthz(cnt)) ;
    MZ(cnt,com)          = MZ(cnt,com)*(1+growthz(cnt)) ;

    TRADEZ(com,cnt,cntt) = TRADEZ(com,cnt,cntt)*(1+growthz(cnt)) ;

    ITZ(cnt)             = ITZ(cnt)*(1+growthz(cnt)) ;

*    KZ(cnt,sec)          =   KZ(cnt,sec)*(1+growthz(cnt)) ;
    LZ(cnt,sec)          =   LZ(cnt,sec)*(1+growthz(cnt)) ;

    KLZ(cnt,sec)         =   KLZ(cnt,sec)*(1+growthz(cnt)) ;

    CZ(cnt,com)          = CZ(cnt,com)*(1+growthz(cnt)) ;
    CBUDZ(cnt)           = CBUDZ(cnt)*(1+growthz(cnt)) ;

    YZ(cnt)              = YZ(cnt)*(1+growthz(cnt)) ;
    SHZ(cnt)             = SHZ(cnt)*(1+growthz(cnt)) ;
    SGZ(cnt)             = SGZ(cnt)*(1+growthz(cnt)) ;
    SROWZ(cnt)           = SROWZ(cnt)*(1+growthz(cnt)) ;
    STZ(cnt)             = STZ(cnt)*(1+growthz(cnt)) ;
    IZ(cnt,com)          = IZ(cnt,com)*(1+growthz(cnt)) ;

    CGZ(cnt,com)         = CGZ(cnt,com)*(1+growthz(cnt)) ;
    TAXRZ(cnt)           = TAXRZ(cnt)*(1+growthz(cnt)) ;

    TRFZ(cnt)            = TRFZ(cnt)*(1+growthz(cnt)) ;

    TRROWZ(cnt)          = TRROWZ(cnt)*(1+growthz(cnt)) ;

    HTRROWZ(cnt)         = HTRROWZ(cnt)*(1+growthz(cnt)) ;

    GDPZ(cnt)            = GDPZ(cnt)*(1+growthz(cnt)) ;

    SVZ(cnt,com)         = SVZ(cnt,com)*(1+growthz(cnt)) ;
    IOZ(cnt,com,sec)     = IOZ(cnt,com,sec)*(1+growthz(cnt)) ;

    NEGVKYZ(cnt,sec)     = NEGVKYZ(cnt,sec)*(1+growthz(cnt)) ;
...
* ========== Fix variables with initial levels zero ============================
    ...
    RK.FX(cnt,sec)$(KZ(cnt,sec) eq 0)            =   1    ;
...
*tc(cnt,'C_FUEL')$EU(cnt) =  tcz(cnt,'C_FUEL') + 0.1 ;
...
walras(t) =           sum(cnt, sum(com,PM.L(cnt,com)*M.L(cnt,com)*MZ(cnt,com))
...
* =================== Calculate results ========================================
    ...
    P_index(cnt,com,t)          =  (P.L(cnt,com)/PZ(cnt,com) -1)*100 ;
    PD_index(cnt,sec,t)         =  (PD.L(cnt,sec)/PDZ(cnt,sec) -1)*100 ;
    PDDE_index(cnt,com,t)       =  (PDDE.L(cnt,com)/PDDEZ(cnt,com) -1)*100 ;
    PXDDE_index(cnt,com,t)      =  (PXDDE.L(cnt,com)/PXDDEZ(cnt,com) -1)*100 ;
    PDD_index(cnt,sec,com,t)    =  (PDD.L(cnt,sec,com)/PDDZ(cnt,sec,com) -1)*100 ;
    ER_index(t)                 =  (ER.L/ERZ -1)*100 ;
    INDEX_index(cnt,t)          =  (INDEX.L(cnt)/INDEXZ(cnt) -1)*100 ;
    PI_index(cnt,t)             =  (PI.L(cnt)/PIZ(cnt) -1)*100 ;

    PEROW_index(cnt,com,t)      =  (PEROW.L(cnt,com)/PEROWZ(cnt,com) -1)*100 ;
    PMROW_index(cnt,com,t)      =  (PMROW.L(cnt,com)/PMROWZ(cnt,com) -1)*100 ;

    PE_index(cnt,com,t)         =  (PE.L(cnt,com)/PEZ(cnt,com) -1)*100 ;
    PM_index(cnt,com,t)         =  (PM.L(cnt,com)/PMZ(cnt,com) -1)*100 ;

    PL_index(cnt,t)             =  (PL.L(cnt)/PLZ(cnt) -1)*100 ;
    PKL_index(cnt,sec,t)        =  (PKL.L(cnt,sec)/PKLZ(cnt,sec) -1)*100 ;
    RGD_index(cnt,t)            =  (RGD.L(cnt)/RGDZ(cnt) -1)*100 ;
    RK_index(cnt,sec,t)         =  (RK.L(cnt,sec)/RKZ(cnt,sec) -1)*100 ;

    LS_index(cnt,t)             =  (LS.L(cnt) -1)*100 ;

    X_index(cnt,com,t)          =  (X.L(cnt,com) -1)*100 ;
    XD_index(cnt,sec,t)         =  (XD.L(cnt,sec) -1)*100 ;
    XDD_index(cnt,sec,com,t)    =  (XDD.L(cnt,sec,com) -1)*100 ;

    XDDE_index(cnt,com,t)       =  (XDDE.L(cnt,com) -1)*100 ;
    XXD_index(cnt,com,t)        =  (XXD.L(cnt,com) -1)*100 ;

    TMX_index(cnt,com,t)        =  (TMX.L(cnt,com) -1)*100 ;

    EROW_index(cnt,com,t)       =  (EROW.L(cnt,com) -1)*100 ;
    MROW_index(cnt,com,t)       =  (MROW.L(cnt,com) -1)*100 ;

    E_index(cnt,com,t)          =  (E.L(cnt,com) -1)*100 ;
    M_index(cnt,com,t)          =  (M.L(cnt,com) -1)*100 ;

    TRADE_index(com,cnt,cntt,t) =  (TRADE.L(com,cnt,cntt) -1)*100 ;

    IT_index(cnt,t)             =  (IT.L(cnt) -1)*100 ;

    K_index(cnt,sec,t)          =  (K.L(cnt,sec) -1)*100 ;
    L_index(cnt,sec,t)          =  (L.L(cnt,sec) -1)*100 ;

    C_index(cnt,com,t)          =  (C.L(cnt,com) -1)*100 ;
    CBUD_index(cnt,t)           =  (CBUD.L(cnt) -1)*100 ;

    Y_index(cnt,t)              =  (Y.L(cnt) -1)*100 ;
    SH_index(cnt,t)             =  (SH.L(cnt) -1)*100 ;
    SG_index(cnt,t)             =  (SG.L(cnt) -1)*100 ;
    SROW_index(cnt,t)           =  (SROW.L(cnt) -1)*100 ;
    ST_index(cnt,t)             =  (ST.L(cnt) -1)*100 ;
    I_index(cnt,com,t)          =  (I.L(cnt,com) -1)*100 ;

    CG_index(cnt,com,t)         =  (CG.L(cnt,com) -1)*100 ;
    TAXR_index(cnt,t)           =  (TAXR.L(cnt) -1)*100 ;

    TRF_index(cnt,t)            =  (TRF.L(cnt) -1)*100 ;

    TRROW_index(cnt,t)          =  (TRROW.L(cnt) -1)*100 ;

    HTRROW_index(cnt,t)         =  (HTRROW.L(cnt) -1)*100 ;

    GDP_index(cnt,t)            =  (GDP.L(cnt) -1)*100 ;
    GDPC_index(cnt,t)           =  (GDPC.L(cnt) -1)*100 ;
    GDPDEF_index(t)              =  (GDPDEF.L/1 -1)*100 ;

    PTM_index(cnt,t)            =  (PTM.L(cnt)/PTMZ(cnt) -1)*100 ;

    SV_index(cnt,com,t)         =  (SV.L(cnt,com) -1)*100 ;
    IO_index(cnt,com,sec,t)     =  (IO.L(cnt,com,sec) -1)*100 ;
