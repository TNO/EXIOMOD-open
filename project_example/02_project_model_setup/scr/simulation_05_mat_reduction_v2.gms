...
GDP_scen_change_KS('LUX',year)
    = (GDP_scen_change_KS('LUX',year)-1)*0.5 +1 ;
...
* Reduction materials is only for year 2, reg AUT and all industries
if(ord(year) gt 1,

* 1. (Reduction of materials for all sectors)
ioc(prd_mat_input,'AUT','iINDU')
    $( mat_red_change(year) and ioc(prd_mat_input,'AUT','iINDU') )
    = ioc(prd_mat_input,'AUT','iINDU')
        * 0.9 ;
...
ioc('pSERV',reg,ind) = ioc('pSERV',reg,ind) + ioc_missing(reg,ind) ;

ioc_sum_check(reg,ind)
    = sum(prd,ioc(prd,reg,ind))-ioc_sum(reg,ind);
