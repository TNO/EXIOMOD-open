...
09_gas_mix
if(ord(year) gt 10 and ord(year) lt 38,
);
...
***** What we need for 10_elec_subsidy
...
* Take energy efficiency from the reference scenario
eprod(ener,reg_pol_03,ind_pol_03)$ener_eff_change('REF',reg_pol_03,year)
    = eprod(ener,reg_pol_03,ind_pol_03) * (1 / ener_eff_change('REF',reg_pol_03,year) ) ;
...
* Make a linear guess of the price.
if(ord(year) gt 2 ,
...
if(ord(year) gt 3 ,
* Expenential guess of the price. Because prices are increasing expenentially.
    PCO2_V.L(reg_pol_05)$(PCO2_time(reg_pol_05,year-1) ge 0.0001 and PCO2_time(reg_pol_05,year-2) ge 0.0001 and PCO2_time(reg_pol_05,year-3) gt 0)
        = 2.71828182845905**( 2 * log(PCO2_time(reg_pol_05,year-1))
            - log(PCO2_time(reg_pol_05,year-2)) ) ;
