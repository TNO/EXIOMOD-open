...
Parameter
    test_coprodB_increaseH2(reg,year)
    test_coprodB_increaseH2_change(reg,year)
;

test_coprodB_increaseH2(reg,'2011') =coprodB(reg,'pNG',reg,'iH2');
test_coprodB_increaseH2(reg,'2050') =0.5;

test_coprodB_increaseH2(reg,year)$(ord(year) ge 2 and ord(year) le 39)
    = test_coprodB_increaseH2(reg,'2011') +
        (test_coprodB_increaseH2(reg,'2050') - test_coprodB_increaseH2(reg,'2011'))
        * (ord(year)) / (2050-2011)  ;

test_coprodB_increaseH2_change(reg,year)$test_coprodB_increaseH2(reg,year-1)
    = test_coprodB_increaseH2(reg,year) / test_coprodB_increaseH2(reg,year-1) ;


test_coprodB_increaseH2_change(reg,'2011')=1;

Display
    test_coprodB_increaseH2
    test_coprodB_increaseH2_change
;
...
if(ord(year) lt 3,
prodK_change(regg,'iH2',year) = 1 ;
prodL_change(regg,'iH2',year) = 1 ;
);
...
coprodB(reg_pol,'pNG',reg_pol,'iH2')
    = coprodB(reg_pol,'pNG',reg_pol,'iH2') *  test_coprodB_increaseH2_change(reg_pol,year) ;

* rescale pNG production
coprodB(reg_pol,'pNG',reg_pol,ind_no_iH2)
    = coprodB(reg_pol,'pNG',reg_pol,ind_no_iH2)
        / sum((regg,indd_no_iH2),coprodB(reg_pol,'pNG',regg,indd_no_iH2))
        * (1 - coprodB(reg_pol,'pNG',reg_pol,'iH2') ) ;
