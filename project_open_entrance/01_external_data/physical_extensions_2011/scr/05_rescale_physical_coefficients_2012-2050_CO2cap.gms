...
Parameter
    coef_emis_c_year(sce_sce,prd,reg,*,emis,year) emission coefficients for CO2 until 2050
    coef_emis_nc_year(sce_sce,reg,ind,emis,year)  emission coefficients for CO2 until 2050
;

coef_emis_c_year(sce_sce,prd,reg,ind,emis_c,year)
    = coef_emis_c(prd,reg,ind,emis_c) ;
coef_emis_nc_year(sce_sce,reg,ind,emis_nc,year)
    = coef_emis_nc(reg,ind,emis_nc) ;
coef_emis_c_year(sce_sce,prd,reg,fd,emis_c,year)
    = coef_emis_c(prd,reg,fd,emis_c) ;

Display coef_emis_c_year;

* ******************* EDGAR CO2 emissions data 2012 - 2018 *********************


Parameter
    GDP_Worldbank_data(*,*)         GDP in 2010 mln USD by country for 2011-2019
*    GDP_eurostat_data(reg_EUS,year_EUS)    GDP in 2010 mln EUR by country for 2011-2020
    ...
    GDP_eurostat_level(reg,year)    GDP in 2010 mln EUR
    coef_emis_EDGAR_level(reg,year) aggregate CO2 emission coefficient
    coef_emis_eurostat_level(reg,year) aggregate CO2 emission coefficient
    coef_emis_eurostat_edgar_level(reg,year)
    coef_emis_change(sce_sce,reg,*,year)    aggregate change in CO2 emission coefficient
;

*$libinclude xlimport GDP_eurostat_data      %project%\01_external_data\physical_extensions_2011\data\GDP_Eurostat_tm2020.xlsx  Data!a10:k37
...
* Estimate CO2 emissions per unit of GDP
coef_emis_eurostat_level(reg,year)$
    GDP_eurostat_level(reg,year)
        = (eurostat_emis(reg,"All NACE activities plus households",year) / 1000)
            / GDP_eurostat_level(reg,year) ;

* Estimate CO2 emissions per unit of GDP
coef_emis_eurostat_edgar_level(reg,year)
    = coef_emis_EDGAR_level(reg,year) ;
coef_emis_eurostat_edgar_level(reg,year)$coef_emis_eurostat_level(reg,year)
    = coef_emis_eurostat_level(reg,year) ;


* Derive annual change in CO2 emisions per unit of GDP
Loop((reg,year)$(ord(year) and coef_emis_eurostat_edgar_level(reg,year-1)),
    coef_emis_change(sce_sce,reg,ind,year)
        = coef_emis_eurostat_edgar_level(reg,year)
            / coef_emis_eurostat_edgar_level(reg,year-1) ;
...
* Manually change GBR. The upwards bump in 2012 is problematic.
coef_emis_change(sce_sce,"GBR",ind,"2012")
    = 1 ;
coef_emis_change(sce_sce,"GBR",ind,"2013")
    = coef_emis_eurostat_edgar_level("GBR","2013")
            / coef_emis_eurostat_edgar_level("GBR","2011") ;
coef_emis_change(sce_sce,"GBR",fd,"2012")
    = 1 ;
coef_emis_change(sce_sce,"GBR",fd,"2013")
    = coef_emis_eurostat_edgar_level("GBR","2013")
            / coef_emis_eurostat_edgar_level("GBR","2011") ;
...
Display coef_emis_change, coef_emis_eurostat_edgar_level ;

* ********************* EC CO2 emissions data 2020 - 2050 **********************
...
Sets
    ...
    ind_serv(*)     service industries      /iSERV, iTRDI/
    ind_energy_supply(*) electricity and steam sectors /iELCC, iELCG, iELCN, iELCH, iELCW, iELCO, iELCB, iELCS, iELCE, iELCT, iH2, iCOAL, iCOIL, iNGAS, iTRDI/
;
...
coef_emis_c_year(sce_sce,prd,reg,ind,"CO2_c","2011")  = coef_emis_c(prd,reg,ind,"CO2_c") ;
coef_emis_nc_year(sce_sce,reg,ind,"CO2_nc","2011")    = coef_emis_nc(reg,ind,"CO2_nc") ;
coef_emis_c_year(sce_sce,prd,reg,fd,"CO2_c","2011")   = coef_emis_c(prd,reg,fd,"CO2_c") ;
Display coef_emis_c_year, year ;

Loop(year$(ord(year) gt 1 ),
    coef_emis_c_year(sce_sce,prd,reg,ind,"CO2_c",year)
        = coef_emis_c_year(sce_sce,prd,reg,ind,"CO2_c",year-1) * coef_emis_change(sce_sce,reg,ind,year) ;
    ...
    coef_emis_c_year(sce_sce,prd,reg,fd,"CO2_c",year)
        = coef_emis_c_year(sce_sce,prd,reg,fd,"CO2_c",year-1) * coef_emis_change(sce_sce,reg,fd,year) ;
) ;
