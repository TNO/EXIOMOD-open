...
* File:     project_ENVLinkages\00-principal\scr\run_simulation_InternalWS_step3.gms
...
EQPK(reg)..
    KLS_V(reg,"GOS")
    =E=
    sum((regg,ind), KL_V(reg,"GOS",regg,ind) ) ;


EQPK.SCALE(reg)$(KLS_V.L(reg,"GOS") gt 0)
    = KLS_V.L(reg,"GOS") ;

EQPK.SCALE(reg)$(KLS_V.L(reg,"GOS") lt 0)
    = -KLS_V.L(reg,"GOS") ;
...
*PKL_V.FX(reg,"COE")        = PKL_V.L(reg,"COE") ;
*PVA_V.FX(regg,ind)      = PVA_V.L(regg,ind) ;

elasIU_DM(prd,regg,ind) = 0 ;
elasFU_DM(prd,regg)     = 0 ;
elasIMP_ROW(prd,regg)   = 0 ;
elasTRD(prd,regg)       = 0 ;
...
Option decimals = 8 ;
Option nlp = conopt3 ;
CGE_step3_demandeffect.scaleopt = 1 ;
Solve CGE_step3_demandeffect using MCP ;
...
Execute_Unload 'results_step3_demandeffect'
