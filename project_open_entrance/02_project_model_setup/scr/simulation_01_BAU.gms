...
Parameter
    ...
    ioc_loop(prd,reg,ind,year)           technical input coefficients for intermediate
                                # inputs (relation in volume)
    theta_h_les_loop(prd,regg,year)
    coprodB_loop(reg,prd,regg,ind,year)
    phi_dom_loop(prd,reg,ind,year)
;
...
LS(reg)                 = LS_V.L(reg) ;
LS_V.FX(reg)            = LS(reg) * LAB_FORCE_WLO_laag_change_wrld(reg,year) ;
...
GTRF(reg) = GTRF(reg) * POP_gr_AG(reg,year) ;
...
ioc_loop(prd,reg,ind,year) = ioc(prd,reg,ind);
theta_h_les_loop(prd,regg,year) = theta_h_les(prd,regg) ;
coprodB_loop(reg,prd,regg,ind,year) = coprodB(reg,prd,regg,ind) ;
phi_dom_loop(prd,reg,ind,year) = phi_dom(prd,reg,ind) ;
...
* Postprocessing to province level
$include %project%/02_project_model_setup/scr/province_level.gms
