...
SV_ROW_time(prd,regg,year)    =   SV_ROW_V.L(prd,regg) ;
...
TOTCONS_time(regg,year)          = sum(prd, CONS_H_T_time(prd,regg,year) )
                                   + sum(prd, CONS_G_T_time(prd,regg,year) )
                                   + sum(prd, GFCF_T_time(prd,regg,year) ) ;

CONS_H_TT_time(regg,year)        = sum(prd, CONS_H_T_time(prd,regg,year) ) ;
CONS_G_TT_time(regg,year)        = sum(prd, CONS_G_T_time(prd,regg,year) ) ;
INTER_USE_TT_time(regg,ind,year) = sum(prd, INTER_USE_T_time(prd,regg,ind,year) ) ;
INTER_USE_MM_time(prd,regg,year) = sum(ind, INTER_USE_M_time(prd,regg,ind,year) ) ;
INTER_USE_DD_time(prd,regg,year) = sum(ind, INTER_USE_D_time(prd,regg,ind,year) ) ;
