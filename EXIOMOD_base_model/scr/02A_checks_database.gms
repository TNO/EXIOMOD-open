...
accounting matrix (MRSAM) database.
...
Parameters
    SUP_sign(reg_data,ind_data,regg_data,prd_data)  indicator for negative
                                                    # values in the supply table
    SUP_offd(reg_data,ind_data,regg_data,prd_data)  indicator for off-diagonal
                                                    # values in the supply table

    USE_bp_sign(reg_data,prd_data,regg_data,ind_data)  indicator for negative
                                                    # values in the intermediate
                                                    # use table in basic prices
    USE_pp_sign(reg_data,prd_data,regg_data,ind_data)  indicator for negative
                                                    # values in the intermediate
                                                    # use table in producer
                                                    # prices

    BAL_bp_reg(reg_data,full_cat_list)  indicator for disbalanced SAM accounts
                                        # for the regions in basic prices
    BAL_pp_reg(reg_data,full_cat_list)  indicator for disbalanced SAM accounts
                                        # for the regions in producer prices
    BAL_bp_row(row_data)                indicator for disbalanced rest of the
                                        # world accounts in basic prices
    BAL_pp_row(row_data)                indicator for disbalanced rest of the
                                        # world accounts in producer prices
;
...
SUP_sign(reg_data,ind_data,regg_data,prd_data)$(SAM_bp_data("%base_year%",
    "%base_cur%",reg_data,ind_data,regg_data,prd_data,"Value") lt 0)
    = 1 ;
...
USE_bp_sign(reg_data,prd_data,regg_data,ind_data)$(SAM_bp_data("%base_year%",
    "%base_cur%",reg_data,prd_data,regg_data,ind_data,"Value") lt 0)
    = 1 ;
...
BAL_bp_reg(reg_data,full_cat_list)
    = SUM((all_reg_data,full_catt_list),
...
* Due to possible rounding, allowed level of discrepancy is 1e-7
BAL_bp_row(row_data)
    = SUM((full_cat_list,all_reg_data,full_catt_list),
...
nerrors
    = SUM((reg_data,ind_data,regg_data,prd_data), SUP_sign(reg_data,
