* File:   %project%/02_project_model_setup/scr/simulation_02_mat_reduction.gms
...
$ontext startdoc
This simulation file decreases the use of product group pINDU and pMANU in
each industry, and replaces this product by a service product pSERV.

This file does not include the baseline scenario with a.o. growth in GDP and
...
Parameters
    ...
    prodL_change(regg,ind,year)          Change in labour productivity
                                         # obtained in calibrate fprod loop
    ioc_sum(reg,ind)                     Sum over all products intermediate
                                         # use table
    ioc_missing(reg,ind)                 Reduction in material consumption that
                                         # is later added to services
    ioc_sum_check(reg,ind)               Check that old sum (ioc_sum) still
                                         # holds after the changes to the ioc
                                         # table
...
sets
* Apply the material reduction only for industries in these regions:
    reg_pol(reg)
/ AUT, BEL, BGR, HRV, CZE, CYP, DNK, EST, FIN, FRA, DEU, GRC, HUN, IRL, ITA, LVA
LTU, LUX, MLT, NLD, POL, PRT, ROU, SVK, SVN, ESP, SWE /

* Reduction of materials, implies reduction of use of following product groups:
    prd_mat_input(prd)
/pINDU, pALUM  /
...
ioc(prd_mat_input,reg_pol,ind)
    $( mat_red_scen_change(year) and ioc(prd_mat_input,reg_pol,ind) )
    = ioc(prd_mat_input,reg_pol,ind)
        * mat_red_scen_change(year) ;
...
* Check that industries are not becoming more efficient. ioc_sum_check(reg,ind)
